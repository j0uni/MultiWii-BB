...
// out message   multitype + multiwii version + protocol version + capability variable 
#define MSP_STATUS    101 
// out message   cycletime & errors_count & sensor present & box activation & current setting number 
...
#define MSP_BAT     110 
// out message   vbat, powermetersum 
#define MSP_RC_TUNING   111 
// out message   rc rate, rc expo, rollpitch rate, yaw rate, dyn throttle PID 
#define MSP_PID     112 
// out message   up to 16 P I D (8 are used) 
#define MSP_BOX     113 
// out message   up to 16 checkbox (11 are used) 
#define MSP_MISC     114 
// out message   powermeter trig + 8 free for future use 
...
// in message   up to 16 P I D (8 are used) 
#define MSP_SET_BOX    203 
// in message   up to 16 checkbox (11 are used) 
	...
	for (n=0;n<UART_NUMBER;n++) 
	{
		...
		while (SerialAvailable (CURRENTPORT) GPS_COND SPEK_COND) 
		{
			...
			if (c_state[CURRENTPORT] == IDLE) 
			{
				...
				if (c_state[CURRENTPORT] == IDLE) evaluateOtherData (c) ; 
...
if (c_state[CURRENTPORT] == HEADER_M) 
			{
				c_state[CURRENTPORT] = (c=='<') ? HEADER_ARROW : IDLE; 

			}
			else
if (c_state[CURRENTPORT] == HEADER_ARROW) 
			{
...
if (c_state[CURRENTPORT] == HEADER_SIZE) 
			{
...
if (c_state[CURRENTPORT] == HEADER_CMD && offset[CURRENTPORT] < dataSize[CURRENTPORT]) 
			{
				checksum[CURRENTPORT] ^= c; 
...
void evaluateCommand () 
{
	switch (cmdMSP[CURRENTPORT]) 
	{
		...
		GPS_speed = read16 () ; 
		GPS_update |= 2;    
		// New data signalisation to GPS functions 
		headSerialReply (0) ; 
		break; 
		#endif 
		case MSP_SET_PID: 
		...
		#endif 
		f.ARMED<<BOXARM) ; 
		serialize8 (global_conf.currentSet) ; 
		// current setting 
		break; 
		case MSP_RAW_IMU: 
		headSerialReply (18) ; 
...

		{
			...
			if (wp_no == 0) 
			{
				...
				serialize16 (0) ;     
				// altitude will come here 
				serialize8 (0) ;     
				// nav flag will come here 
...
ISR (USART_UDRE_vect) 
{
	...
	ISR (USART0_UDRE_vect) 
	{
		...
		if (t == serialHeadTX[0]) UCSR0B &= ~ (1<<UDRIE0) ; 
	...
	{
		// Serial 1 on a MEGA or on a PROMICRO 
	...
	static void inline store_uart_in_buf (uint8_t data, uint8_t portnum) 
	{
		...
		if (portnum == SPEK_SERIAL_PORT) 
		{
			if (!spekFrameFlags) 
			{
				...
				// timer0_overflow_count will be slightly off because of the way the Arduino core timer interrupt handler works; that is acceptable for this use. Using the core variable avoids an expensive call to millis () or micros () 
				spekTimeLast = spekTimeNow; 

				if (spekInterval > 5000) 
				{
					// Potential start of a Spektrum frame, they arrive every 11 or every 22 ms. Mark it, and clear the buffer. 
